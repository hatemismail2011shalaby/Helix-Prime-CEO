...
def find_files_by_name(query: str, roots: Optional[Iterable[Path]] = None, max_results: int = 100) -> List[str]:
...
def normalize_query_to_paths(query: str) -> List[Path]:
    """Convert common query text into a list of candidate root paths."""
    roots: List[Path] = []
    lower = query.lower()
    if "h:" in lower or "drive h" in lower or "h drive" in lower:
        roots.append(Path("H:/"))
